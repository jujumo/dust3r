...
# command lives in docker/files/train_overmaps_entrypoint.sh — edit that file on the
...
#
# Usage:
#   bash train_overmaps.sh [--cpu] [--engine=docker|podman]
...
exec $compose_cmd -f "$compose_file" run --rm \
    ...
    --entrypoint /dust3r/docker/files/train_overmaps_entrypoint.sh \
    dust3r-demo
